...
aria.Utils.isFocusable = function (element) {
  ...
  aria.Dialog = function (dialogId, focusAfterClosed, focusFirst) {
    ...
    var item = document.getElementById("tabItemOne");
    if (item) {
        item.focus();
    }
  ...
  window.openDialog = function (dialogId, focusAfterClosed, focusFirst) {
    ...
    var right_pane = document.getElementById('right');
    right_pane.style.display = 'none';
    dlg.style.float="right";
/*
    try{
      if (dlg) {
        dlg.style.zIndex = 5000;//"10";
      }
      console.log(dlg.style.zIndex);
    }
    catch (err) {
      console.log(err);
    }
...
function loadLocalServerParams () {
  ...
  try {
    var remoteData = localStorage.getItem(remoteParamsStorage);
    if (remoteData == null) {
      remoteData = setRemoteDefaults();
      localStorage.setItem(remoteParamsStorage, JSON.stringify(remoteData));
    }
    if (remoteData) {
      jsonRemoreParams = JSON.parse(remoteData);
    }
    console.log(remoteData);
    return (jsonRemoreParams);
  }
  catch (err) {
    jsonRemoreParams = {};

    jsonRemoreParams['server'] = 'localhost';
    jsonRemoreParams['port'] = '4567';
    localStorage.setItem(remoteParamsStorage, JSON.stringify(jsonRemoreParams));
  }
  return (jsonRemoreParams);
}
//-----------------------------------------------------------------------------
function setRemoteDefaults() {
  var jsonBumpsRemoteServer = {};

  jsonBumpsRemoteServer['server'] = 'localhost';
  jsonBumpsRemoteServer['port'] = '4567';
  return (jsonBumpsRemoteServer);
}
//-----------------------------------------------------------------------------
